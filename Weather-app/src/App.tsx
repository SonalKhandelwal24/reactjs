...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (storedCities && storedCities.length > 0) {
      storedCities.forEach((city: City) => {
  ...
  const fetchWeatherData = async (citydata: City) => {
    try {
      ...
      setWeatherData(prevWeatherData => ({
        ...prevWeatherData,
        [citydata.city]: { city: citydata, data }
      }));
  ...
  // {  'New Delhi': { city: { city: 'New Delhi', country: 'India' }, data: {/* some data */} },
  //   'Mumbai': { city: { city: 'Mumbai', country: 'India' }, data: {/* some data */} } }
  // setWeatherData(prevWeatherData => ({
  //   ...prevWeatherData,
  //   'Bangalore': { city: { city: 'Bangalore', country: 'India' }, data }
  // }));
  ...
  const handleAddCity = () => {
    if (selectedCity && !addedCities.find(city => city.city === selectedCity.city)) {
  ...
  const handleDeleteCity = (cityToDelete: City) => {
    const updatedCities = addedCities.filter(city => city.city !== cityToDelete.city);
    setAddedCities(updatedCities);
    localStorage.setItem('addedCities', JSON.stringify(updatedCities));
